...
- `to_css.py` — Non-default brands ([data-brand=X] on Kernel's `.krnl-companion-layer` wrapper) didn't actually re-skin anything beyond the raw `--brand-*`/`--font-*` vars — every semantic/typography/foundations/component alias that depends on them (action.*, status.*, surface.*, text.*, data-tone.*, property.*, typography.* font-family, 223 vars total) stayed frozen to Corilus
  - Before: `:root { --action-solid: var(--brand-primary-deep); ... } [data-brand="X"] { --brand-primary-deep: #...; }` — no re-declaration, so buttons/status/surfaces/typography never changed brand even though the raw --brand-* vars did
  - After: `to_css.py` computes the transitive closure of every :root alias whose var() chain resolves through brand.*/font.*, and re-emits those 223 declarations inside each `[data-brand=X]` block (auto-discovered from `brand/*.tokens.json`, not hand-maintained)
  - *Affects:* every component that reads a semantic-tier token (buttons, status/feedback, surfaces, text, data-viz, patient-property colours, typography font-family) — i.e. effectively the whole system, for any brand other than the Corilus default
  - *Rationale:* A CSS custom property's `var()` reference resolves against the value of the referenced property AT THE ELEMENT WHERE THE REFERENCING DECLARATION ITSELF LIVES. `--action-solid` is declared only at `:root` (→ `<html>`), while the brand override lands on a descendant wrapper — so the reference locked onto whatever brand was active at `<html>` (always Corilus) and that resolved value simply inherited down unchanged. Confirmed empirically switching to Sofia in Storybook: `--brand-primary-deep` updated correctly on the wrapper, `--action-solid` did not. Discovered while verifying the Sofia brand swap; this bug predates it and would have identically affected Semble/myNeva.
  - *Migration:* None for consumers — same var names, now actually reflecting the selected brand. Regenerate via `python3 tokens/to_css.py tokens > tokens/tokens.css`.
